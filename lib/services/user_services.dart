import 'package:quiz/repository/db_repository.dart';
import 'package:quiz/models/user_model.dart';
import 'package:crypto/crypto.dart';
import 'package:quiz/repository/user_repository.dart';
import 'dart:convert';
...
// Login
  ...
  Future<UserModel?> searchUser(String email) async {
    UserRepository userRepository = UserRepository();
    final List<Map<String, dynamic>> maps =
        await userRepository.searchUser(email);
    if (maps.isNotEmpty) {
      final userList = List.generate(maps.length, (i) {
        return UserModel(
          name: maps[i]['name'],
          ...
          coin: maps[i]['coin'],
          gems: maps[i]['gems'],
          imagePath: maps[i]['imagePath'] ?? "lib/assets/logo/user.png",
        );
      });
      return userList[0];
    }
  ...
  Future updateUserProfile() async {
    ...
    userModel.imagePath = userModel.imagePath;

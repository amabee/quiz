...
class QuestionServices {
  ...
  Future<List<QuestionModel>> getQuestionByCategory(int categoryId) async {
    ...
    List<Map<String, dynamic>> maps =
        await questionRepository.loadQuestionByCategory(categoryId);
    if (maps.isNotEmpty) {
      final questionList = List.generate(maps.length, (i) {
        return QuestionModel(
            category: maps[i]['category'],
            categoryId: maps[i]['categoryId'],
            hasImage: maps[i]['hasImage'] == 1 ? true : false,
  ...
  Future<List<QuestionModel>> getQuestionByRandom() async {
    QuestionRepository questionRepository = QuestionRepository();

    List<Map<String, dynamic>> maps =
        await questionRepository.loadQuestionByRandom();
    if (maps.isNotEmpty) {
      final questionList = List.generate(maps.length, (i) {
        return QuestionModel(
            category: maps[i]['category'],
